...
def preprocess_data(filename=None, save_path='Home1/train_data.csv'):
    import os
    import numpy as np
    raw_train_data = []
    ...
    if not filename:
        ...
        _, cut_data = read_file(filename)
        raw_train_data.extend(cut_data)
    timed_raw_train_data = sort_by_time(raw_train_data)
    # test
    # for i in range(len(timed_raw_train_data)):
    #     if timed_raw_train_data[i][0] == "1625222000.260051" or timed_raw_train_data[i][0] == "1625222336.14939":
    #         print(timed_raw_train_data[i], i)
    #         break
    ...
    print(training_samples.shape, not_change_count)
    np.savetxt(save_path, training_samples, delimiter=',')
